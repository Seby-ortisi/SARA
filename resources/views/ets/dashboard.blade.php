...
@endif
   ...
   <div class="text-center mb-4">
        <h2 class="border-bottom pb-3">{{ __('ETS - Modulo registrazione utente') }}</h2>
        <p class="lead">
            {{ $dataView['ets_nome']->descrizione ?? "Nessun ETS associato"}}
        </p>
    </div>
    ...
    <form action="@if(empty($dataView['user']->id)) {{ route('ets.registraNuovoUtente') }} @else {{ route('admin.aggiornaDati', ['id' => $dataView['user']->id]) }} @endif" method="POST" enctype="multipart/form-data">
        @csrf
    ...
    <select id="ets" name="ets" class="form-select form-select-sm w-100">
        <option value="0">Nessun ETS</option>

        @foreach ($dataView['ets'] as $ets)
            <option value="{{ $ets->id }}"
                {{ (string) old('ets', optional($dataView['ets_user'])->ets_id) === (string) $ets->id ? 'selected' : '' }}>
                {{ $ets->descrizione }}
            </option>
        @endforeach
    </select>
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, item) {
                            ...
                            <li class="list-group-item">
                                <form method="POST" action="{{ route('ets.inserisciPazienteAPC') }}" id="${item.codfis}">
                                    @csrf
